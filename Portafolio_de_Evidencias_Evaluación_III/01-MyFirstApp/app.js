...
// 2. Usamos la herramienta 'figlet' para crear tipografia ASCII
figlet('Diseno Digital', function(error, textoDibujado){
    if (error){
        console.log('Hubo un error al dibujar la tipografia.');
        return;
    }
...
console.log(chalk.yellow('¡Hola desde mi computadora, sin necesidad de usar el navegador!'));
console.log(chalk.bgRed.white('NPM es mi nueva App Store'));


console.log(chalk.bgBlue.white("EDGAR VILLANUEVA"));

});


figlet('BRAGHENN', function(error, textoDibujado){
    if (error){
        console.log('Hubo un error al dibujar la tipografia.');
        return;
    }

console.log(chalk.red.bold(textoDibujado));
